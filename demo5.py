import time
import math
import random
from dataclasses import dataclass

import pybullet as p
import pybullet_data


# ============================================================
# 1) THAM SỐ CHỈNH NHANH
# ============================================================

DT = 1.0 / 240.0

# ---------- Conveyor ----------
CONVEYOR_SPEED = 0.30                   # tốc độ băng tải, số nhỏ để dễ bắt
SPAWN_Y = -0.60                         # vị trí sinh vật thể                     
DESPAWN_Y = 0.90                        # vị trí hủy vật thể
SPAWN_INTERVAL = 1.20                   # thời gian tối thiểu giữa 2 lần sinh
MIN_SPAWN_DY = 0.11                     # khoảng cách tối thiểu giữa 2 vật thể khi sinh

# ---------- Robot speed ----------
SPEED_SCALE = 0.80                      # tỉ lệ tốc độ chung (1.0 = bình thường, <1.0 chậm hơn)

# ---------- Motion steps ----------
MOVE_STEPS_HOVER = 85                   # số bước di chuyển đến hover
MOVE_STEPS_DESCEND = 40                 # số bước đi xuống pick
MOVE_STEPS_LIFT = 60                    # số bước nâng lên sau pick
MOVE_STEPS_TO_BIN = 115                 # số bước đi đến bin
MOVE_STEPS_HOME_SETTLE = 140            # số bước về home và đợi ổn định

# ---------- Early stop ----------
EE_TOL_HOVER = 0.016                    # khoảng cách dừng sớm khi đến hover
EE_TOL_PLACE = 0.013                    # khoảng cách dừng sớm khi đến place
EE_SETTLE_STEPS = 8                     # số bước ổn định sau khi dừng sớm

# ---------- Prediction / Tracking ----------
TRACK_XY_GAIN = 1.0                     # hệ số gain điều chỉnh xy khi track    
PRED_TRACK_STEPS = 6                    # số bước dự đoán khi track
PRED_HOVER_STEPS = 14                   # số bước dự đoán khi đến hover

# ---------- Gripper (track + close) ----------
GRIP_HOLD_FORCE = 240                   # lực giữ kẹp khi đã kẹp được
GRIP_CLOSE_STEP = 0.0015                # bước đóng kẹp mỗi vòng lặp
GRIP_STUCK_EPS = 1e-4                   # ngưỡng kẹp bị kẹt
GRIP_STUCK_COUNT = 4                    # số vòng lặp kẹt để dừng
REQUIRE_CONTACT = True                  # yêu cầu có tiếp xúc mới kẹp thành công

# ---------- Attach ----------
USE_ATTACH = True                       # sử dụng attach thay vì physics kẹp
ATTACH_MAX_EE_DIST = 0.035              # khoảng cách tối đa EE -> object để attach
ATTACH_MAX_FORCE = 3500                 # lực tối đa của attach

# ---------- Gripper yaw ----------
GRIP_YAW = math.pi / 2                  # hướng kẹp (0 = dọc theo x robot, pi/2 = dọc theo y robot)

# ---------- Place safety (mới) ----------
# Bay cao để không quẹt hàng trước, rồi đi xuống thẳng đứng
PLACE_CLEAR_Z = 0.14                    # độ cao an toàn trên mặt đặt (m)
PLACE_PRE_RELEASE_Z = 0.030   # nhích lên trước khi mở kẹp (m)
PLACE_RETRACT_Z = 0.10        # rút thẳng lên sau khi mở kẹp (m)
PLACE_LINEAR_STEPS_DOWN = 55            # số bước đi xuống thẳng
PLACE_LINEAR_STEPS_UP = 35              # số bước đi lên thẳng

# ---------- Debug ----------
DEBUG_PRINT_EVERY = 60                  # in debug mỗi n bước
DEBUG_OVERLAY = True                    # hiển thị debug overlay
DEBUG_LINES = True                      # hiển thị debug lines


# ============================================================
# 2) WORKSPACE / ROI / BINS
# ============================================================

PICK_ROI = {                            # vùng chọn vật thể trên băng tải
    "xmin": 0.30, "xmax": 0.75,
    "ymin": -0.10, "ymax": 0.25,
    "z_pick": 0.020,
    "z_hover": 0.18,
}

BINS = {                                # vị trí đặt vật thể theo màu
    "red":    (0.60, 0.40, 0.02),
    "green":  (0.30, 0.40, 0.02),
    "blue":   (0.00, 0.50, 0.02),
    "yellow": (-0.30, 0.20, 0.02),
}

WAYPOINT_MID = (0.45, 0.35, 0.22)       # điểm trung gian an toàn khi đi đến bin


# ============================================================
# 3) ROBOT CONSTANTS + OBJECT SIZE (pallet)
# ============================================================

EE_LINK = 11                            # link cuối cánh tay (end-effector)
ARM_JOINTS = list(range(7))             # các khớp cánh tay    
GRIPPER_JOINTS = [9, 10]                # các khớp kẹp
HOME_Q = [0.0, -0.4, 0.0, -2.2, 0.0, 2.0, 0.8]      # tư thế home

EE_DOWN_ORN = None                      # hướng end-effector khi xuống (tính sau)

OBJ_HALF = 0.02
OBJ_SIZE = 2.0 * OBJ_HALF

# ✅ tăng gap 1 chút để giảm va chạm hàng-hàng khi xếp
OBJ_GAP = 0.010
CELL = OBJ_SIZE + OBJ_GAP               # kích thước cell pallet
LAYER_H = OBJ_SIZE + 0.005

PALLET_N = 4

COLORS = {
    "red":    (1, 0, 0, 1),
    "green":  (0, 1, 0, 1),
    "blue":   (0, 0, 1, 1),
    "yellow": (1, 1, 0, 1),
}


def ee_down_yaw(yaw_rad: float):
    return p.getQuaternionFromEuler([math.pi, 0.0, yaw_rad])


# ============================================================
# 4) DATA STRUCTURES
# ============================================================

@dataclass
class Obj:
    body_id: int
    color_name: str
    sorted: bool = False


# ============================================================
# 5) DEBUGGER
# ============================================================

class Debugger:
    def __init__(self):
        self._text_ids = {}
        self._keys_order = []

    def overlay(self, key, text, color=(1, 0, 1), size=1.2):
        if not DEBUG_OVERLAY:
            return

        if key not in self._keys_order:
            self._keys_order.append(key)
        idx = self._keys_order.index(key)

        anchor = [0.9, -0.6, 1.0]
        pos = [anchor[0], anchor[1], anchor[2] - 0.03 * idx]

        if key in self._text_ids:
            try:
                p.removeUserDebugItem(self._text_ids[key])
            except Exception:
                pass

        self._text_ids[key] = p.addUserDebugText(
            text, pos, textColorRGB=color, textSize=size, lifeTime=0
        )

    def line(self, a, b, rgb=(1, 0, 1), width=2, life=0.15):
        if not DEBUG_LINES:
            return
        p.addUserDebugLine(a, b, rgb, lineWidth=width, lifeTime=life)

    def draw_roi_and_bins(self):
        x0, x1 = PICK_ROI["xmin"], PICK_ROI["xmax"]
        y0, y1 = PICK_ROI["ymin"], PICK_ROI["ymax"]
        z = PICK_ROI["z_pick"]

        pts = [(x0, y0, z), (x1, y0, z), (x1, y1, z), (x0, y1, z)]
        for i in range(4):
            p.addUserDebugLine(pts[i], pts[(i + 1) % 4], [1, 0, 1], lineWidth=2, lifeTime=0)

        for name, (bx, by, bz) in BINS.items():
            p.addUserDebugText(
                name, (bx, by, bz + 0.06),
                textColorRGB=COLORS[name][:3],
                textSize=1.4,
                lifeTime=0
            )


# ============================================================
# 6) GRID
# ============================================================

def draw_world_grid(
    x_min=-0.6, x_max=0.9,
    y_min=-0.9, y_max=0.9,
    step=0.1,
    z=0.001,
    major_every=5,
    show_axes=True,
    show_labels=True,
    label_every=0.2
):
    i = 0
    x = x_min
    while x <= x_max + 1e-9:
        is_major = (i % major_every == 0)
        color = [0.7, 0.7, 0.7] if is_major else [0.5, 0.5, 0.5]
        width = 2.5 if is_major else 1.0
        p.addUserDebugLine([x, y_min, z], [x, y_max, z], color, width, lifeTime=0)
        i += 1
        x += step

    i = 0
    y = y_min
    while y <= y_max + 1e-9:
        is_major = (i % major_every == 0)
        color = [0.7, 0.7, 0.7] if is_major else [0.5, 0.5, 0.5]
        width = 2.5 if is_major else 1.0
        p.addUserDebugLine([x_min, y, z], [x_max, y, z], color, width, lifeTime=0)
        i += 1
        y += step

    if show_axes:
        p.addUserDebugLine([0, 0, z], [0.5, 0, z], [1, 0, 0], 4, lifeTime=0)
        p.addUserDebugText("X+", [0.52, 0, z], [1, 0, 0], textSize=1.4, lifeTime=0)
        p.addUserDebugLine([0, 0, z], [0, 0.5, z], [0, 1, 0], 4, lifeTime=0)
        p.addUserDebugText("Y+", [0, 0.52, z], [0, 1, 0], textSize=1.4, lifeTime=0)

    if show_labels and label_every:
        x = x_min
        while x <= x_max + 1e-9:
            if abs((x / label_every) - round(x / label_every)) < 1e-6:
                p.addUserDebugText(f"x={x:.1f}", [x, 0, z], [0.9, 0.9, 0.9], textSize=0.9, lifeTime=0)
            x += step

        y = y_min
        while y <= y_max + 1e-9:
            if abs((y / label_every) - round(y / label_every)) < 1e-6:
                p.addUserDebugText(f"y={y:.1f}", [0, y, z], [0.9, 0.9, 0.9], textSize=0.9, lifeTime=0)
            y += step


# ============================================================
# 7) PALLETIZER
# ============================================================

class Palletizer:
    """
    4x4 nhiều tầng, so le chuẩn theo pattern 4 tầng lặp.
    ✅ FIX: fill theo hàng xa -> hàng gần để giảm bay qua hàng đã đặt.
    """
    def __init__(self):
        self.state = {c: {"count": 0} for c in BINS.keys()}

    def _layer_shift(self, layer_idx: int):
        hx = 0.5 * CELL
        hy = 0.5 * CELL
        k = layer_idx % 4
        if k == 0:
            return (0.0, 0.0)
        if k == 1:
            return (hx, 0.0)
        if k == 2:
            return (0.0, hy)
        return (hx, hy)

    def next_place_pose(self, color_name: str):
        cnt = self.state[color_name]["count"]
        layer = cnt // (PALLET_N * PALLET_N)
        idx_in_layer = cnt % (PALLET_N * PALLET_N)

        row_raw = idx_in_layer // PALLET_N
        col = idx_in_layer % PALLET_N
        row = (PALLET_N - 1) - row_raw  # ✅ hàng xa -> hàng gần

        ox, oy, oz = BINS[color_name]

        cx = (col - (PALLET_N - 1) / 2.0) * CELL
        cy = (row - (PALLET_N - 1) / 2.0) * CELL
        sx, sy = self._layer_shift(layer)

        px = ox + cx + sx
        py = oy + cy + sy
        pz = oz + OBJ_HALF + layer * LAYER_H

        place = [px, py, pz]
        hover = [px, py, pz + 0.16]
        return place, hover, (layer, row, col, cnt)

    def commit(self, color_name: str):
        self.state[color_name]["count"] += 1


# ============================================================
# 8) CONVEYOR WORLD
# ============================================================

class ConveyorWorld:
    def __init__(self, dbg: Debugger):
        self.objects: list[Obj] = []
        self.dbg = dbg
        self.sim_time = 0.0
        self.last_spawn_sim = -999.0

    def spawn_if_needed(self):
        if (self.sim_time - self.last_spawn_sim) < SPAWN_INTERVAL:
            return

        for o in self.objects:
            if o.sorted:
                continue
            pos, _ = p.getBasePositionAndOrientation(o.body_id)
            if pos[1] < SPAWN_Y + MIN_SPAWN_DY:
                return

        self.last_spawn_sim = self.sim_time
        self._spawn_object()

    def _spawn_object(self):
        color_name = random.choice(list(COLORS.keys()))
        rgba = COLORS[color_name]

        col_shape = p.createCollisionShape(p.GEOM_BOX, halfExtents=[OBJ_HALF, OBJ_HALF, OBJ_HALF])
        vis_shape = p.createVisualShape(p.GEOM_BOX, halfExtents=[OBJ_HALF, OBJ_HALF, OBJ_HALF], rgbaColor=rgba)

        x = random.uniform(PICK_ROI["xmin"], PICK_ROI["xmax"])
        y = SPAWN_Y
        z = OBJ_HALF

        body = p.createMultiBody(
            baseMass=0.08,
            baseCollisionShapeIndex=col_shape,
            baseVisualShapeIndex=vis_shape,
            basePosition=[x, y, z],
        )

        p.changeDynamics(body, -1, lateralFriction=1.0, rollingFriction=0.02, spinningFriction=0.02)
        p.resetBaseVelocity(body, linearVelocity=[0.0, CONVEYOR_SPEED, 0.0])

        self.objects.append(Obj(body, color_name, sorted=False))

    def update_conveyor(self, held_obj_id=None):
        self.sim_time += DT
        to_remove = []

        for obj in self.objects:
            if obj.sorted:
                continue
            if held_obj_id is not None and obj.body_id == held_obj_id:
                continue

            pos, _ = p.getBasePositionAndOrientation(obj.body_id)
            if pos[1] > DESPAWN_Y:
                to_remove.append(obj)
                continue

            p.resetBaseVelocity(obj.body_id, linearVelocity=[0.0, CONVEYOR_SPEED, 0.0])

        for obj in to_remove:
            try:
                p.removeBody(obj.body_id)
            except Exception:
                pass
            if obj in self.objects:
                self.objects.remove(obj)

    def get_pick_candidate(self):
        cx = 0.5 * (PICK_ROI["xmin"] + PICK_ROI["xmax"])
        cy = 0.5 * (PICK_ROI["ymin"] + PICK_ROI["ymax"])

        best = None
        best_d2 = 1e9

        for obj in self.objects:
            if obj.sorted:
                continue
            (x, y, _), _ = p.getBasePositionAndOrientation(obj.body_id)

            if (PICK_ROI["xmin"] <= x <= PICK_ROI["xmax"]) and (PICK_ROI["ymin"] <= y <= PICK_ROI["ymax"]):
                d2 = (x - cx) ** 2 + (y - cy) ** 2
                if d2 < best_d2:
                    best_d2 = d2
                    best = obj

        return best


# ============================================================
# 9) PANDA ROBOT
# ============================================================

class PandaRobot:
    def __init__(self):
        self.robot_id = p.loadURDF(
            "franka_panda/panda.urdf",
            useFixedBase=True,
            flags=p.URDF_USE_SELF_COLLISION
        )

        self.grasp_cid = None
        self.held_obj = None
        self.world = None

        self.ll, self.ul, self.jr = [], [], []
        for j in ARM_JOINTS:
            info = p.getJointInfo(self.robot_id, j)
            low, high = info[8], info[9]
            self.ll.append(low)
            self.ul.append(high)
            rng = (high - low) if (high > low and abs(high - low) < 10) else 2 * math.pi
            self.jr.append(rng)

    # ---------------- SIM STEP ----------------
    def _step(self, n=1, realtime=False):
        for _ in range(n):
            if self.world is not None:
                self.world.update_conveyor(held_obj_id=self.held_obj)
            p.stepSimulation()
            if realtime:
                time.sleep(DT)

    # ---------------- GRIPPER ----------------
    def open_gripper(self):
        for j in GRIPPER_JOINTS:
            p.setJointMotorControl2(self.robot_id, j, p.POSITION_CONTROL, targetPosition=0.04, force=80)

    def gripper_q(self):
        return tuple(p.getJointState(self.robot_id, j)[0] for j in GRIPPER_JOINTS)

    # ---------------- EE / IK ----------------
    def get_ee_pose(self):
        ls = p.getLinkState(self.robot_id, EE_LINK)
        return ls[4], ls[5]

    def ee_distance_to(self, target_pos):
        ee_pos, _ = self.get_ee_pose()
        dx = ee_pos[0] - target_pos[0]
        dy = ee_pos[1] - target_pos[1]
        dz = ee_pos[2] - target_pos[2]
        return math.sqrt(dx * dx + dy * dy + dz * dz)

    def get_arm_q(self):
        return [p.getJointState(self.robot_id, j)[0] for j in ARM_JOINTS]

    def ik(self, target_pos, target_orn, rest_poses=None):
        if rest_poses is None:
            rest_poses = self.get_arm_q()  # ✅ giảm IK jump

        q = p.calculateInverseKinematics(
            self.robot_id,
            EE_LINK,
            targetPosition=target_pos,
            targetOrientation=target_orn,
            lowerLimits=self.ll,
            upperLimits=self.ul,
            jointRanges=self.jr,
            restPoses=rest_poses,
            maxNumIterations=220,
            residualThreshold=1e-4
        )
        return [q[i] for i in range(7)]

    def goto_ee(self, pos, orn, steps=100, realtime=True, tol=0.01, settle=10, arm_force=420):
        q = self.ik(pos, orn)
        steps = max(10, int(steps * SPEED_SCALE))

        ok_count = 0
        for _ in range(steps):
            for i, j in enumerate(ARM_JOINTS):
                p.setJointMotorControl2(
                    self.robot_id, j,
                    p.POSITION_CONTROL,
                    targetPosition=q[i],
                    force=arm_force
                )

            self._step(1, realtime=realtime)

            if self.ee_distance_to(pos) < tol:
                ok_count += 1
                if ok_count >= settle:
                    break
            else:
                ok_count = 0

    # ✅ NEW: đi thẳng (linear) để không quẹt pallet
    def goto_ee_linear(self, target_pos, orn, steps=60, realtime=True, arm_force=420):
        start_pos, _ = self.get_ee_pose()
        steps = max(10, int(steps * SPEED_SCALE))

        for k in range(steps):
            a = (k + 1) / steps
            pos = [
                start_pos[0] + a * (target_pos[0] - start_pos[0]),
                start_pos[1] + a * (target_pos[1] - start_pos[1]),
                start_pos[2] + a * (target_pos[2] - start_pos[2]),
            ]
            q = self.ik(pos, orn)
            for i, j in enumerate(ARM_JOINTS):
                p.setJointMotorControl2(self.robot_id, j, p.POSITION_CONTROL, targetPosition=q[i], force=arm_force)
            self._step(1, realtime=realtime)

    # ---------------- Prediction / Track ----------------
    def predict_obj_xy(self, obj_id, horizon_steps: int):
        (x, y, _), _ = p.getBasePositionAndOrientation(obj_id)
        y_pred = y + CONVEYOR_SPEED * DT * horizon_steps
        return x, y_pred

    def descend_track(self, obj_id, orn, z_start, z_end, steps=50, realtime=True, arm_force=450):
        steps = max(10, int(steps * SPEED_SCALE))
        for k in range(steps):
            alpha = (k + 1) / steps
            z = z_start + alpha * (z_end - z_start)

            remain = max(1, steps - (k + 1))
            horizon = min(PRED_TRACK_STEPS, remain)
            tx_obj, ty_obj = self.predict_obj_xy(obj_id, horizon)

            ee_pos, _ = self.get_ee_pose()
            tx = ee_pos[0] + TRACK_XY_GAIN * (tx_obj - ee_pos[0])
            ty = ee_pos[1] + TRACK_XY_GAIN * (ty_obj - ee_pos[1])

            q_arm = self.ik([tx, ty, z], orn)
            for i, j in enumerate(ARM_JOINTS):
                p.setJointMotorControl2(self.robot_id, j, p.POSITION_CONTROL, targetPosition=q_arm[i], force=arm_force)

            self._step(1, realtime=realtime)

    # ---------------- Contact ----------------
    def _contacts_lr(self, obj_id):
        cps_left = p.getContactPoints(bodyA=self.robot_id, bodyB=obj_id, linkIndexA=GRIPPER_JOINTS[0])
        cps_right = p.getContactPoints(bodyA=self.robot_id, bodyB=obj_id, linkIndexA=GRIPPER_JOINTS[1])
        return (len(cps_left), len(cps_right))

    # ---------------- Track + Close ----------------
    def track_and_close_until_grasp(self, obj_id, orn, z_pick, max_loops=160, realtime=True):
        target = min(self.gripper_q())
        last_q = None
        stuck_count = 0

        for _ in range(max_loops):
            tx_obj, ty_obj = self.predict_obj_xy(obj_id, PRED_TRACK_STEPS)
            ee_pos, _ = self.get_ee_pose()
            tx = ee_pos[0] + TRACK_XY_GAIN * (tx_obj - ee_pos[0])
            ty = ee_pos[1] + TRACK_XY_GAIN * (ty_obj - ee_pos[1])

            q_arm = self.ik([tx, ty, z_pick], orn)
            for i, j in enumerate(ARM_JOINTS):
                p.setJointMotorControl2(self.robot_id, j, p.POSITION_CONTROL, targetPosition=q_arm[i], force=470)

            target = max(0.0, target - GRIP_CLOSE_STEP)
            for j in GRIPPER_JOINTS:
                p.setJointMotorControl2(self.robot_id, j, p.POSITION_CONTROL, targetPosition=target, force=GRIP_HOLD_FORCE)

            self._step(1, realtime=realtime)

            cl, cr = self._contacts_lr(obj_id)
            if REQUIRE_CONTACT and (cl > 0 or cr > 0):
                hold = min(self.gripper_q())
                for j in GRIPPER_JOINTS:
                    p.setJointMotorControl2(self.robot_id, j, p.POSITION_CONTROL, targetPosition=hold, force=GRIP_HOLD_FORCE)
                return True, hold

            now_q = self.gripper_q()
            if last_q is not None:
                if abs(now_q[0] - last_q[0]) < GRIP_STUCK_EPS and abs(now_q[1] - last_q[1]) < GRIP_STUCK_EPS:
                    stuck_count += 1
                    if stuck_count >= GRIP_STUCK_COUNT:
                        hold = min(now_q)
                        for j in GRIPPER_JOINTS:
                            p.setJointMotorControl2(self.robot_id, j, p.POSITION_CONTROL, targetPosition=hold, force=GRIP_HOLD_FORCE)
                        return True, hold
                else:
                    stuck_count = 0
            last_q = now_q

        return False, min(self.gripper_q())

    # ---------------- Attach no-snap ----------------
    def can_attach(self, obj_id):
        ee_pos, _ = self.get_ee_pose()
        obj_pos, _ = p.getBasePositionAndOrientation(obj_id)
        dist = math.dist(ee_pos, obj_pos)

        if dist > ATTACH_MAX_EE_DIST:
            return False, dist

        cl, cr = self._contacts_lr(obj_id)
        if REQUIRE_CONTACT and (cl == 0 and cr == 0):
            return False, dist

        return True, dist

    def attach_no_snap(self, obj_id, max_force=ATTACH_MAX_FORCE):
        if self.grasp_cid is not None:
            return

        ee_pos, ee_orn = self.get_ee_pose()
        obj_pos, obj_orn = p.getBasePositionAndOrientation(obj_id)

        inv_obj_pos, inv_obj_orn = p.invertTransform(obj_pos, obj_orn)
        child_pos, child_orn = p.multiplyTransforms(inv_obj_pos, inv_obj_orn, ee_pos, ee_orn)

        self.grasp_cid = p.createConstraint(
            parentBodyUniqueId=self.robot_id,
            parentLinkIndex=EE_LINK,
            childBodyUniqueId=obj_id,
            childLinkIndex=-1,
            jointType=p.JOINT_FIXED,
            jointAxis=[0, 0, 0],
            parentFramePosition=[0, 0, 0],
            parentFrameOrientation=[0, 0, 0, 1],
            childFramePosition=child_pos,
            childFrameOrientation=child_orn
        )

        p.changeConstraint(self.grasp_cid, maxForce=max_force)
        self.held_obj = obj_id

    def detach(self):
        if self.grasp_cid is None:
            return
        try:
            p.removeConstraint(self.grasp_cid)
        except Exception:
            pass
        self.grasp_cid = None
        self.held_obj = None

    # ---------------- Home ----------------
    def reset_home(self):
        self.detach()
        self.open_gripper()

        for i, j in enumerate(ARM_JOINTS):
            p.resetJointState(self.robot_id, j, HOME_Q[i])

        for i, j in enumerate(ARM_JOINTS):
            p.setJointMotorControl2(self.robot_id, j, p.POSITION_CONTROL, targetPosition=HOME_Q[i], force=420)

        self._step(MOVE_STEPS_HOME_SETTLE, realtime=True)

    def go_home_smooth(self, steps=220, realtime=True):
        self.detach()
        self.open_gripper()
        for i, j in enumerate(ARM_JOINTS):
            p.setJointMotorControl2(self.robot_id, j, p.POSITION_CONTROL, targetPosition=HOME_Q[i], force=420)
        self._step(int(steps * SPEED_SCALE), realtime=realtime)


# ============================================================
# 10) LOG + PICK & PLACE
# ============================================================

def fmt3(v):
    return f"({v[0]:.3f},{v[1]:.3f},{v[2]:.3f})"

def pick_log(stage, robot: PandaRobot, obj_id=None, extra=""):
    ee_pos, _ = robot.get_ee_pose()
    gq = robot.gripper_q()
    if obj_id is None:
        print(f"[{stage}] ee={fmt3(ee_pos)} gq=({gq[0]:.4f},{gq[1]:.4f}) {extra}")
        return

    obj_pos, _ = p.getBasePositionAndOrientation(obj_id)
    dist = math.dist(ee_pos, obj_pos)
    cl, cr = robot._contacts_lr(obj_id)
    print(f"[{stage}] ee={fmt3(ee_pos)} obj={fmt3(obj_pos)} dist={dist:.3f} "
          f"gq=({gq[0]:.4f},{gq[1]:.4f}) contact(L,R)=({cl},{cr}) held={robot.held_obj} {extra}")


def pick_and_place(robot: PandaRobot, obj: Obj, ee_orn, dbg: Debugger, pallet: Palletizer) -> bool:
    obj_id = obj.body_id

    # ---------------- PICK ----------------
    xh, yh = robot.predict_obj_xy(obj_id, PRED_HOVER_STEPS)
    hover = [xh, yh, PICK_ROI["z_hover"]]
    dbg.line(hover, [xh, yh, PICK_ROI["z_pick"]], rgb=(1, 1, 0), width=3)

    pick_log("PICK:start", robot, obj_id=obj_id, extra=f"color={obj.color_name} pred_hover={PRED_HOVER_STEPS}")

    robot.open_gripper()
    robot.goto_ee(hover, ee_orn, steps=MOVE_STEPS_HOVER, realtime=True, tol=EE_TOL_HOVER, settle=EE_SETTLE_STEPS)
    pick_log("PICK:hover", robot, obj_id=obj_id)

    robot.descend_track(obj_id, ee_orn, PICK_ROI["z_hover"], PICK_ROI["z_pick"], steps=MOVE_STEPS_DESCEND, realtime=True)
    pick_log("PICK:descend_done", robot, obj_id=obj_id)

    ok_grasp, hold_q = robot.track_and_close_until_grasp(obj_id, ee_orn, PICK_ROI["z_pick"], max_loops=180, realtime=True)
    pick_log("PICK:track_close", robot, obj_id=obj_id, extra=f"ok={ok_grasp} hold_q={hold_q:.5f}")
    if not ok_grasp:
        robot.open_gripper()
        robot._step(12, realtime=True)
        return False

    ok_attach, dist = robot.can_attach(obj_id)
    cl, cr = robot._contacts_lr(obj_id)
    print(f"[ATTACH_CHECK] ok={ok_attach} dist={dist:.3f} contact(L,R)=({cl},{cr}) USE_ATTACH={USE_ATTACH}")

    if USE_ATTACH and ok_attach:
        robot.attach_no_snap(obj_id)
        pick_log("PICK:attached", robot, obj_id=obj_id)
    else:
        robot.open_gripper()
        robot._step(10, realtime=True)
        return False

    ee_pos, _ = robot.get_ee_pose()
    robot.goto_ee([ee_pos[0], ee_pos[1], PICK_ROI["z_hover"]], ee_orn, steps=MOVE_STEPS_LIFT, realtime=True,
                 tol=EE_TOL_HOVER, settle=EE_SETTLE_STEPS)
    pick_log("PICK:lift_done", robot, obj_id=obj_id)

    # ---------------- PLACE (PALLET SAFE PATH) ----------------
    place, _, meta = pallet.next_place_pose(obj.color_name)
    layer, row, col, cnt = meta

    # 1) đi qua waypoint mid (giảm singularity)
    robot.goto_ee(list(WAYPOINT_MID), ee_orn, steps=int(MOVE_STEPS_TO_BIN * 0.65), realtime=True, tol=0.02, settle=EE_SETTLE_STEPS)

    # 2) bay tới điểm clear_above (cao), rồi đi xuống THẲNG đứng
    clear_above = [place[0], place[1], max(PICK_ROI["z_hover"], place[2] + PLACE_CLEAR_Z)]
    robot.goto_ee_linear(clear_above, ee_orn, steps=55, realtime=True, arm_force=420)

    # 3) hạ xuống gần mặt đặt, rồi xuống place (đều bằng linear để không quẹt ngang)
    pre_release = [place[0], place[1], place[2] + PLACE_PRE_RELEASE_Z]
    robot.goto_ee_linear(pre_release, ee_orn, steps=PLACE_LINEAR_STEPS_DOWN, realtime=True, arm_force=420)
    robot.goto_ee_linear(place, ee_orn, steps=max(18, int(PLACE_LINEAR_STEPS_DOWN * 0.55)), realtime=True, arm_force=420)

    pick_log("PLACE:at_cell", robot, obj_id=obj_id, extra=f"idx={cnt} layer={layer} r={row} c={col} place={fmt3(place)}")

    # 4) ✅ chống húc hàng: NHÍCH LÊN rồi mới detach + mở kẹp
    robot.goto_ee_linear(pre_release, ee_orn, steps=18, realtime=True, arm_force=420)

    robot.detach()
    robot._step(2, realtime=True)  # cho physics ổn định 1 nhịp trước khi mở
    robot.open_gripper()
    robot._step(12, realtime=True)

    # 5) làm vật đứng yên trong pallet
    p.resetBaseVelocity(obj_id, linearVelocity=[0, 0, 0], angularVelocity=[0, 0, 0])
    p.changeDynamics(
        obj_id, -1,
        lateralFriction=1.2, rollingFriction=0.02, spinningFriction=0.02,
        linearDamping=0.7, angularDamping=0.9
    )
    obj.sorted = True
    pallet.commit(obj.color_name)

    # 6) rút thẳng lên cao rồi mới bay đi chỗ khác
    retract = [place[0], place[1], max(clear_above[2], place[2] + PLACE_RETRACT_Z)]
    robot.goto_ee_linear(retract, ee_orn, steps=PLACE_LINEAR_STEPS_UP, realtime=True, arm_force=420)

    pick_log("PLACE:done", robot, obj_id=obj_id)
    return True


# ============================================================
# 11) MAIN
# ============================================================

def main():
    global EE_DOWN_ORN

    p.connect(p.GUI)
    p.setAdditionalSearchPath(pybullet_data.getDataPath())
    p.setGravity(0, 0, -9.81)
    p.setTimeStep(DT)

    EE_DOWN_ORN = ee_down_yaw(GRIP_YAW)

    p.loadURDF("plane.urdf")

    draw_world_grid(
        x_min=-0.6, x_max=0.9,
        y_min=-0.9, y_max=0.9,
        step=0.1,
        major_every=5,
        show_axes=True,
        show_labels=True,
        label_every=0.2
    )

    p.resetDebugVisualizerCamera(
        cameraDistance=1.25,
        cameraYaw=55,
        cameraPitch=-35,
        cameraTargetPosition=[0.42, 0.2, 0.15]
    )

    dbg = Debugger()
    dbg.draw_roi_and_bins()

    world = ConveyorWorld(dbg)
    robot = PandaRobot()
    robot.world = world
    pallet = Palletizer()

    robot.reset_home()

    sim_step = 0
    busy = False
    last_action_status = "idle"

    try:
        while True:
            sim_step += 1

            if not busy:
                world.spawn_if_needed()

            if not busy:
                cand = world.get_pick_candidate()
                if cand is not None:
                    busy = True
                    ok = pick_and_place(robot, cand, EE_DOWN_ORN, dbg, pallet)
                    last_action_status = "OK" if ok else "FAIL_GRASP"
                    robot.go_home_smooth(steps=220, realtime=True)
                    busy = False

            if sim_step % DEBUG_PRINT_EVERY == 0:
                ee_pos, _ = robot.get_ee_pose()
                gq = robot.gripper_q()
                cand2 = world.get_pick_candidate()

                cand_str = "None"
                if cand2 is not None:
                    cpos, _ = p.getBasePositionAndOrientation(cand2.body_id)
                    cand_str = f"id={cand2.body_id} color={cand2.color_name} pos=({cpos[0]:.3f},{cpos[1]:.3f},{cpos[2]:.3f})"

                print(f"[step={sim_step}] sim_time={world.sim_time:.2f} objs={len(world.objects)} busy={busy} held={robot.held_obj} "
                      f"ee={fmt3(ee_pos)} gripper=({gq[0]:.4f},{gq[1]:.4f}) cand={cand_str} status={last_action_status}")

                dbg.overlay("A", f"step: {sim_step} | sim_time: {world.sim_time:.2f}")
                dbg.overlay("B", f"objects: {len(world.objects)} | busy: {busy}")
                dbg.overlay("C", f"held_obj: {robot.held_obj}")
                dbg.overlay("D", f"ee: ({ee_pos[0]:.3f}, {ee_pos[1]:.3f}, {ee_pos[2]:.3f})")
                dbg.overlay("E", f"gripper: ({gq[0]:.4f},{gq[1]:.4f})")
                dbg.overlay("F", f"conv={CONVEYOR_SPEED} pred_hover={PRED_HOVER_STEPS} pred_track={PRED_TRACK_STEPS}")
                dbg.overlay("G", f"last: {last_action_status}",
                            color=(0, 1, 0) if last_action_status == "OK" else (1, 0.2, 0.2))

            robot._step(1, realtime=True)

    except KeyboardInterrupt:
        pass
    finally:
        p.disconnect()


if __name__ == "__main__":
    main()
