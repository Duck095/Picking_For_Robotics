...
class RewardModulePlace:
    ...
    def __init__(
        ...
        target_pos,
        time_penalty=0.01,
        dist_weight=0.6,          # ✅ tăng để kéo vật về target rõ hơn
        release_bonus=0.2,        # ✅ nhỏ thôi, tránh spam nhả
        success_bonus=3.0,
        success_dist=0.12,
        table_z=0.06,
        delta_clip=0.03,          # ✅ clip progress
        z_release_max=None,       # 3C: bắt buộc thả thấp (vd 0.08)
        high_release_penalty=1.0, # phạt thả quá cao
    ...
    def compute(self, robot_id: int, obj_id: int, grip: float, holding: bool):
        ...
        # --- detect released event: holding True -> False ---
        released = False
