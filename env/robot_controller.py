...
class PandaController:
    EE_LINK = 11                     # ✅ panda_grasptarget confirmed
    ...
    def __init__(self, robot_id: int, grip_yaw=math.pi / 2):
        ...
        # ✅ hướng kẹp: thử pi/2 hoặc 0 để đúng “ngang” bạn muốn
    ...
    def apply_action(
        self, dx, dy, dz, grip,
        arm_force=350,
        # ✅ chống cắm xuống: đừng cho TCP xuống quá thấp
        z_min=0.03, z_max=0.55,
        x_min=0.10, x_max=0.85,
        y_min=-0.60, y_max=0.60
    ):
        tx = min(x_max, max(x_min, self.target_pos[0] + float(dx)))
        ty = min(y_max, max(y_min, self.target_pos[1] + float(dy)))
        tz = min(z_max, max(z_min, self.target_pos[2] + float(dz)))
