...
class PlaceDebugLoggerCallback(BaseCallback):
    """
    Ghi 2 file CSV:
    ...
    def __init__(self, debug_csv_path: str, summary_csv_path: str, n_envs: int, verbose=0):
        ...
        self.episode_min_obj_target_dists = [float("inf") for _ in range(n_envs)]
        self.episode_start_obj_target_dists = [None for _ in range(n_envs)]
        self.episode_holding_steps = [0 for _ in range(n_envs)]
    ...
    def _reset_episode_stats(self, env_idx: int):
        ...
        self.episode_min_obj_target_dists[env_idx] = float("inf")
        self.episode_start_obj_target_dists[env_idx] = None
        self.episode_holding_steps[env_idx] = 0
    ...
    def _on_step(self) -> bool:
        ...
        for env_idx in range(len(infos)):
            ...
            target_pos = self._safe_tuple3(info.get("target_pos"))
            ...
            if isinstance(action, (list, tuple)) and len(action) >= 4:
                action_x, action_y, action_z, action_grip = action[0], action[1], action[2], action[3]
            else:
                action_x, action_y, action_z, action_grip = None, None, None, None

            ee_obj_dist = info.get("ee_obj_dist", None)
            obj_target_dist = info.get("obj_target_dist", None)
            success = bool(info.get("success", False))
            holding = bool(info.get("holding", False))
            attached_now = bool(info.get("attached_now", False))
            release_cooldown = info.get("release_cooldown", None)

            if self.episode_start_obj_target_dists[env_idx] is None and obj_target_dist is not None:
                self.episode_start_obj_target_dists[env_idx] = obj_target_dist

            if obj_target_dist is not None:
                self.episode_min_obj_target_dists[env_idx] = min(
                    self.episode_min_obj_target_dists[env_idx], obj_target_dist
                )
            ...
            if holding:
                self.episode_holding_steps[env_idx] += 1

            delta_obj_target_x = None if (obj_pos[0] is None or target_pos[0] is None) else (target_pos[0] - obj_pos[0])
            delta_obj_target_y = None if (obj_pos[1] is None or target_pos[1] is None) else (target_pos[1] - obj_pos[1])
            delta_obj_target_z = None if (obj_pos[2] is None or target_pos[2] is None) else (target_pos[2] - obj_pos[2])
            ...
            debug_row = {
                ...
                "place_substage": info.get("place_substage", info.get("substage", "")),
                "reward": reward,
                "success": success,
                "done": done,
                "holding": holding,
                ...
                "delta_obj_target_x": delta_obj_target_x,
                "delta_obj_target_y": delta_obj_target_y,
                "delta_obj_target_z": delta_obj_target_z,
            }
            ...
            if done:
                ...
                ep_start_obj_target_dist = self.episode_start_obj_target_dists[env_idx]
                ep_final_obj_target_dist = obj_target_dist
                ep_min_obj_target_dist = (
                    None if self.episode_min_obj_target_dists[env_idx] == float("inf")
                    else self.episode_min_obj_target_dists[env_idx]
                )
                ep_success = int(success)
                ep_mean_reward = ep_total_reward / ep_len if ep_len > 0 else None
                holding_ratio = (
                    self.episode_holding_steps[env_idx] / ep_len if ep_len > 0 else None
                )
                ...
                summary_row = {
                    ...
                    "episode_start_obj_target_dist": ep_start_obj_target_dist,
                    "episode_final_obj_target_dist": ep_final_obj_target_dist,
                    "episode_min_obj_target_dist": ep_min_obj_target_dist,
                    "episode_success": ep_success,
                    "holding_ratio": holding_ratio,
                    "final_ee_x": ee_pos[0],
                    "final_ee_y": ee_pos[1],
                    "final_ee_z": ee_pos[2],
                    "final_obj_x": obj_pos[0],
                    "final_obj_y": obj_pos[1],
                    "final_obj_z": obj_pos[2],
                    "target_x": target_pos[0],
                    "target_y": target_pos[1],
                    "target_z": target_pos[2],
