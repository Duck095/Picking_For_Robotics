...
class GraspDebugSummaryCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if done:
            ...
            self.xy_dist_window.append(float(info.get("xy_dist", -1.0)))
            self.z_dist_window.append(float(info.get("z_dist", -1.0)))
            ...
            line = (
                ...
                f"home_hold_mean_{self.window_size}={self._safe_mean(self.home_hold_steps_window):.2f} "
                f"home_err_mean_{self.window_size}={self._safe_mean(self.home_error_window):.4f}"
            )
